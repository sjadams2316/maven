...
1. **Signal Normalization**
   ...
   - Trading (LONG/SHORT): → -1 to +1
   - Forecast (up/down): → -1 to +1
...
`DivergenceScanner.tsx` — Full/compact modes with:
- Visual bar showing social (purple) vs analyst (blue) positions
- Severity badges (⚠️ Major, 🔔 Moderate, 💡 Minor)
- Expandable cards with detailed analysis
- Auto-refresh support

### Advisor Page

`/partners/analysis/divergence` — Full scanner UI with:
...
1. ✅ Signal Consensus View
2. ✅ **Divergence Scanner** (NEW)
3. ✅ Portfolio Pulse
4. ⏳ Smart Tax Harvester (sentiment + loss)
5. ⏳ Earnings Sentiment Tracker
6. ⏳ Sector Heatmap
...
```
        Query Arrives
              │
        ┌─────▼─────┐
        │   GROQ    │  ← Classifies in ~10-50ms
        │  (fast)   │
        └─────┬─────┘
              │
        ┌─────▼─────┐
        │  ROUTER   │  ← Picks: speed / cost / deep
        └─────┬─────┘
              │
    ┌─────────┼─────────┐
    │         │         │
┌───▼───┐ ┌───▼───┐ ┌───▼───┐
│  LLM  │ │  xAI  │ │ VANTA │  ← PARALLEL FETCH
│Groq/  │ │Twitter│ │Trading│
│Chutes │ │       │ │Signals│
└───┬───┘ └───┬───┘ └───┬───┘
    │         │         │
    └─────────┼─────────┘
        ...
        └─────┬─────┘
              │
        ┌─────▼─────┐
        │ RESPONSE  │  ← Smarter + cheaper
        └───────────┘
```

### New Files

- `orchestrator.ts` — The main brain (~500 lines)
- `/api/athena/orchestrate` — Test endpoint
...
## 8:30 PM - Perplexity Research Provider (`9d00055`)

Sam's insight: "Perplexity is a game changer for advisors."

**Why it matters:**
- Real-time research, not stale data
- **Cited sources** — crucial for compliance and trust
- Advisors can answer "What's happening with X?" instantly
- Curate research for clients
...
```typescript
// Quick research
const result = await perplexityResearch("What's happening with NVDA?");
// → { content: "...", citations: [{url, title}...], latencyMs: 2000 }

// Stock-specific
const stock = await researchStock("NVDA", { focus: "earnings" });
// → { summary, keyPoints, sentiment, citations }

// Market/macro
const market = await researchMarket("Fed rate decision");
// → { content, citations }
```
...
```
GET /api/athena/research?q=Your question
GET /api/athena/research?symbol=NVDA&focus=earnings
GET /api/athena/research?topic=Fed rate decision

Focus options: news, earnings, analysis, general
Recency options: hour, day, week, month
```

### Orchestrator Integration

Deep/research path now routes to Perplexity:
```
Query classified as "research"
→ Router picks "deep" path
→ Perplexity fetches with citations
→ Response includes sources
```

### Athena Provider Status

| Provider | Status | Use Case |
|----------|--------|----------|
| **Groq** | ✅ | Classification, speed path |
| **Chutes** | ✅ | Cost path (95% savings) |
| **Perplexity** | ✅ NEW | Research with citations |
| **xAI** | ✅ | Twitter sentiment |
| **Vanta** | ⏳ | Trading signals (needs key) |

**Perplexity key added to Vercel ✅**


---

## 8:45 PM - Claude as Synthesis Brain (`f61cab6`)

Sam's insight: "Think about how Claude could be a multiplier effect."

**THE ARCHITECTURAL SHIFT:**

```
BEFORE: Claude = just another LLM option
AFTER:  Claude = THE BRAIN that synthesizes everything
```
...
### Claude Synthesis Function

```typescript
const result = await claudeSynthesize({
  query: "Should I sell NVDA?",
  initialResponse: "...", // From Chutes
  research: { content: "...", citations: [...] }, // From Perplexity
  sentiment: [{ symbol: "NVDA", direction: "bullish", ... }], // From xAI
  tradingSignals: [{ symbol: "NVDA", direction: "LONG", ... }], // From Vanta
  context: { holdings: ["NVDA", "AAPL"], riskTolerance: "moderate" }
});
```

**Claude's system prompt:**
> "You are Maven Oracle's synthesis brain. Your job is to take multiple intelligence sources and produce unified, actionable wisdom for wealth management."

### The Multiplier Effect

| Model | Role | Cost |
|-------|------|------|
| Groq | Fast classification, simple chat | Free tier |
| Chutes | Bulk analysis | 95% cheaper |
| Perplexity | Research | ~$0.02/query |
| xAI | Twitter sentiment | ~$0.005/query |
| **Claude** | **SYNTHESIZES ALL OF THE ABOVE** | ~$0.01/query |

Cheap models do 90% of the work. Claude does the 10% that matters most.
...
**Athena is now a true hybrid intelligence network.**
...
```
┌─────────────────────────────────────────────────────┐
│  🧠 NVDA Signal Consensus              [Refresh]    │
├─────────────────────────────────────────────────────┤
│                                                     │
│   [🟢]  BULLISH                                     │
│         Score: +0.42                                │
│                                                     │
│   Confidence: 72%  │  Agreement: STRONG             │
│   ████████████░░░  │  ████████████████░░           │
│                                                     │
│   ◄─────────────●───────────────────────────►      │
...
│                                                     │
├─────────────────────────────────────────────────────┤
│  CONTRIBUTING SOURCES (3)                           │
│                                                     │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐   │
│  │ Twitter(xAI)│ │ Reddit     │ │ Vanta       │   │
│  │ 🟢 +0.45    │ │ 🟢 +0.32   │ │ 🟢 +0.60    │   │
...
├─────────────────────────────────────────────────────┤
│  ⚡ ALERTS                                          │
│  • Strong bullish signal across sources             │
│                                                     │
├─────────────────────────────────────────────────────┤
│  🧠 REASONING                                       │
│  ✓ Overall assessment is moderately bullish        │
│  ✓ Based on 3 sources: 2 sentiment, 1 trading      │
...
This is the **advisor view** — shows everything Athena knows about a symbol.
